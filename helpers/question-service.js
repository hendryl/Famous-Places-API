...
var query = 'SELECT places.place_id, places.name, places.description, places.link, places.latitude, places.longitude, places.photo_id, locations.country, locations.flag, locations.continent FROM places LEFT JOIN (SELECT countries.country_id, countries.name AS "country", countries.image AS "flag", continents.continent_id, continents.name AS "continent" FROM countries JOIN continents on countries.continent_id = continents.continent_id) AS locations ON places.country_id = locations.country_id subquery ORDER BY places.place_id';
